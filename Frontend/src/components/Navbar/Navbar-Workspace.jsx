import {FaChevronDown, FaPlus, FaThLarge} from 'react-icons/fa';
import {IoIosSearch, IoIosNotifications, IoIosHelpCircle,} from "react-icons/io";
import {Link} from "react-router-dom";
import {TemplateSVG} from "../Icon/icons.jsx";
import {BsGrid1X2Fill} from "react-icons/bs";

export default function NavbarWorkspace() {
    return (
        <nav className="flex items-center justify-between bg-white p-3 shadow-md w-full">
            {/* Left Section */}
          ...
          <span className="cursor-pointer hover:text-black flex items-center gap-2 text-gray-600 font-semibold text-md">
            ...
            <div className="flex items-center gap-4">
                <div className="relative flex items-center hidden md:flex">
                    <IoIosSearch className="absolute left-3 text-gray-500 w-5 h-5"/>
                    <input
                        type="text"
                        placeholder="Search"
                        className="pl-10 pr-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-300"
                    />
                </div>
                ...
                <div className="w-7 h-7 bg-red-500 text-white rounded-full flex items-center justify-center font-bold">
                    GH
                </div>
            </div>
        </nav>
